...
get_pod_info() {
  ...
  while IFS=$'\t' read -r node_name instance_type spot_label; do
    ...
    # Use space as delimiter for fields within the map value
 ...
 do
    ...
    node_info="${node_info_map[$node_name]:-"unknown false"}"
    # Read will split based on space by default
    read -r instance_type is_spot <<< "$node_info"
    ...
    # Print the pod name, node name, instance type, and namespace
    printf "% -40s % -65s % -30s %s\n" "$pod_name" "$node_name" "$instance_type_output" "$namespace"
  done
}
...
echo "-------------------------------------------------------------------------------------------------------------------------------------------------------"
printf "% -40s % -65s % -30s %s\n" "POD NAME" "NODE NAME" "NODE TYPE" "NAMESPACE"
echo "-------------------------------------------------------------------------------------------------------------------------------------------------------"
